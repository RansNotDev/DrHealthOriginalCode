...
<style>
    nav {
    background-color: #0D409E; /* Updated navigation bar color */
...
#closeModal:hover, #closeModalBottom:hover {
...
@media print {
    ...
    .sidebar, nav, button, .hidden, #availability-container, .section.hidden {
        display: none;
    }
    ...
    html, body, #queueModal {
        page-break-inside: avoid;
        page-break-before: avoid;
        page-break-after: avoid;
    }

    #closeModal, #closeModalBottom, #printButton {
        display: none;
    }
}
</style>
...
document.getElementById('appdate').addEventListener('change', function() {
    ...
    if (selectedDate) {
        ...
        if (availableDates.includes(selectedDate)) {
            // Fetch availability again for the selected date
            fetchAvailability(selectedDoctor, selectedDate, data => {
                const apptimeSelect = document.getElementById('apptime');
            ...
            // AND set the "app-submit" button to disabled
